<#
.SYNOPSIS
    Opens a pull request for the current (or named) branch; -Push pushes it first.
.EXAMPLE
    ./scripts/New-PullRequest.ps1 -Repo "MyRepo" -Title "Fix login" -Description "..." -TargetBranch main -Push
#>
[CmdletBinding()]
param(
    [Parameter(Mandatory)][string]$Repo,
    [Parameter(Mandatory)][string]$Title,
    [string]$Description = '',
    [string]$SourceBranch,
    [string]$TargetBranch,
    [switch]$Draft,
    [switch]$Push,
    [string]$Reviewers,
    [string]$Labels
)
$ErrorActionPreference = 'Stop'
$CadenceApi = if ($env:CADENCE_API) { $env:CADENCE_API } else { 'http://127.0.0.1:3800' }
$headers = @{}
if ($env:CADENCE_TOKEN) { $headers['x-cadence-token'] = $env:CADENCE_TOKEN }
function Get-Api($path) { Invoke-RestMethod -Uri "$CadenceApi$path" -Headers $headers }
function Post-Api($path, $payload) { Invoke-RestMethod -Uri "$CadenceApi$path" -Method Post -Headers $headers -ContentType 'application/json' -Body ($payload | ConvertTo-Json -Depth 8) }
function Esc($s) { [uri]::EscapeDataString([string]$s) }
# Arrays always print as JSON arrays, an empty one included, so a caller can parse the output blindly.
function Out-Json($o, $d = 6) { ConvertTo-Json -InputObject $o -Depth $d }
$payload = @{ repo = $Repo; title = $Title; body = $Description; draft = [bool]$Draft; push = [bool]$Push }
if ($SourceBranch) { $payload.head = $SourceBranch }
if ($TargetBranch) { $payload.base = $TargetBranch }
if ($Reviewers) { $payload.reviewers = @($Reviewers -split ',' | ForEach-Object { $_.Trim() } | Where-Object { $_ }) }
if ($Labels) { $payload.labels = @($Labels -split ',' | ForEach-Object { $_.Trim() } | Where-Object { $_ }) }
Post-Api '/api/github/pulls/create' $payload | ConvertTo-Json
