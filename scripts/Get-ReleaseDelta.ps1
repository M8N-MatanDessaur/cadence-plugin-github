<#
.SYNOPSIS
    Commits and pull requests since a tag (default: the latest release), for release notes.
.EXAMPLE
    ./scripts/Get-ReleaseDelta.ps1 -Repo "MyRepo" -From v1.2.0
#>
[CmdletBinding()]
param(
...
Get-Api "/api/github/releases/delta?repo=$(Esc $Repo)&from=$(Esc $From)&to=$(Esc $To)" | ConvertTo-Json -Depth 6
